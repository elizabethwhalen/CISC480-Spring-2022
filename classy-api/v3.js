...
app.post('/v2/login', async function (req, res) {
    ...
    //check database for record matching provided username
    try{ loginjson = await login_query_db("SELECT * from login where user_id="+con.escape(req.body.username),res);
    } catch(e){
        return res.status(e[0]).send(e[1]);
    }
    //if no record exists return 404 error
    if (loginjson.length==0){return res.status(404).send("Username not found")};

    //extract hashed password from matching record in database and verify it matches submitted password
    passHashed = loginjson[0].pass
    if (passHashed === undefined) { return res.sendStatus(401); } //can this happen?
    ...
    .then(correct => {
      if(correct){
        ...
        let token = jwt.sign(
          ...
          {
            algorithm: "HS256",
            expiresIn: 43200, //5 minutes
          });
        res.cookie("token", token, { maxAge: 43200 * 1000 }) //5 minutes
...
function login_query_db_post(query, data, res) {
  new Promise((resolve, reject) => {
      con.query(query, data, (err, result) => {
          if (err) {
              reject([400,"Bad Request"]); //CHANGETHIS
          } else {
              resolve(result)
          }
      });
      ...
      }).catch(res.status(500).send("Error querying database"));
}
async function login_query_db(query,res){
